            ...
            return (
              <div key={task.id} className="px-5 py-3.5 flex items-center gap-4 hover:bg-gray-50 transition-colors">
                <div className="flex-1 min-w-0">
                  ...
                  {task.next_step && (
                    <p className="text-xs text-gray-400 truncate mt-0.5">→ {task.next_step}</p>
                  )}
                </div>
                {client && (
                  <span className="text-xs font-medium flex-shrink-0" style={{ color: client.color }}>
                    {client.name}
                  </span>
                )}
                ...
                <span className={`text-xs px-2.5 py-1 rounded-lg font-medium flex-shrink-0 ${badge.cls}`}>
                  {badge.label}
                </span>
              </div>
            )
